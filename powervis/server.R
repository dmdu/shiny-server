...
shinyServer(function(input, output) {
  ...
  output$analysis <- renderPrint({
    ...
    if (nrow(power_all) > 0) {
      # Select and prune
      power = power_filter(range_selection(power_all, start=input$SelectionRange[1], end=input$SelectionRange[2]),input$LowThresh,input$HighThresh)
  ...
  output$Scatter <- renderPlot({
    ...
    if (enough_data) {
      ...
      #qplot(power$Time, power$Power) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + geom_smooth() + geom_point() 
